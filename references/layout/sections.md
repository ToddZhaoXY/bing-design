...
- Sections taller than 424px use expand/collapse container (M/S only), collapsed height = **424px**.
...
- Zone A = full-bleed content (e.g. map, no padding). Height = 4 rows (424px).
- Zone C = 2 cards maximum — no text labels, no headings.
- Expand/collapse only at M/S, collapsed height = **424px**.

### Height variants

| Height variant | Behavior |
|---|---|
| `default` | Zone A always 4 rows (424px) at all breakpoints |
| `fallback` | Zone A drops to 2 rows (200px) at col 4 / col 2, giving more space to Zone C |
...
- Always expandable — collapsed height = **424px** (via `max-height`), expanded up to 3000px.
- Zone C cards fixed at **200px** height.
...
| Section | Zones | Expand/Collapse | Collapsed Height | Zone C at M/S |
|---|---|---|---|---|
| CGM | A+B+C | M/S only | 424px | horizontal row |
| Algo | A+C | No | — | stacked below |
| Wide Answer | A+C (50/50) | M/S only | 424px | stacked below |
| AIG | A+C | Always | 424px (`max-height`) | **hidden** |
